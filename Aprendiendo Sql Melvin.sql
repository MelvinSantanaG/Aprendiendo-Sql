...
--2 - Objetivos y alcances del tutorial de Microsoft Sql Server: 
--El curso brinda un concepto terico corto, 
--luego un problema resuelto que invito a ejecutar, modificar y 
--jugar con el mismo. Por último, y lo más importante, 
--una serie de ejercicios propuestos que nos permitirté saber
...
--3 - Crear una tabla (create table - sp_tables - sp_columns - drop table)
 ...
 create table usuarios ( 
   ...
   exec sp_columns usuarios drop table usuarios
...
--11 - Comentarios
 ...
 select titulo, autor /*mostramos tútulos y nombres de los autores*/
 from libros
...
--15 - Otras caracterésticas del atributo Identity
 if object_id('libros') is not null
  drop table libros
...
--64 - go: es un signo de finalización de un lote de sentencias SQL. No es una sentencia, es un comando.
    --El lote de sentencias esté compuesto por todas las sentencias antes de "go" o todas las sentencias entre dos "go".
...
--65 - Vistas
 ...
 insert into secciones values('Administración',300)
 insert into secciones values('Contadurúa',400)
...
--75 - Funciones con valores de tabla en l�nea
 if object_id('libros') is not null
  drop table libros
